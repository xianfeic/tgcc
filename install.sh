...
echo 'alias tgcc="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/xianfeic/tgcc" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/xianfeic/tgcc'"
...
rm -rf "$HOME/.config/@tgcc"
rm -rf "$HOME/.config/@tgcc"
rm -rf "$HOME/.config/tgcc-core"
